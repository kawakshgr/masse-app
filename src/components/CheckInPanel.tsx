...
const cell =
  "mt-1 h-8 w-full rounded-r2 border border-[var(--edge)] bg-[var(--glass2)] px-2 text-[13px] text-[var(--ink)]";
    ...
    <fieldset className="min-w-[180px] flex-1">
      ...
      <div className="mt-1 flex gap-1">
        {values.map((value) => (
          <label
            key={value}
            className="flex h-8 min-w-0 flex-1 cursor-pointer items-center justify-center rounded-rp border border-[var(--edge)] bg-[var(--glass2)] px-1 text-[12px] text-[var(--ink2)] has-checked:border-[var(--accent)] has-checked:text-[var(--accent)]"
          >
            <input
              type="radio"
              name={name}
