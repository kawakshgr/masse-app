    ...
    <form action={addFood} className="flex flex-wrap items-end gap-2">
      ...
      {(
        ...
        <label key={field} className="block w-[78px]">
          <span className="block text-[10px] text-[var(--ink3)]">{label}</span>
          <input
            name={field}
            inputMode="decimal"
            value={macros[key]}
      ...
      <div className="w-[96px]">
        <span className="block text-[10px] text-[var(--ink3)]">{t("kcal")}</span>
        <p className="tnum mt-1 flex h-8 items-center justify-center rounded-r2 border border-dashed border-[var(--edge)] text-[12px] font-semibold text-[var(--ink2)]">
          {kcal}
        </p>
      </div>
      ...
      <button
        type="submit"
        className="h-8 shrink-0 rounded-rp bg-[var(--accent)] px-4 text-[12px] font-semibold text-[var(--on-accent)]"
      >
        {t("add")}
      </button>
