-- The coach sees the phase and the levers, never anything that dates a period.
--
-- Her Cycle tab promises that period dates are never shown to her and that the
-- database refuses them. Two things broke that promise through this function:
--
-- 1. day_of_cycle (added 20260923180000) went to the coach too. Day 3 on the
--    24th is a period that started on the 22nd.
-- 2. p_on could be any date. Asking day after day for the phase finds the day
--    it turns menstrual — the same date, recovered by probing.
--
-- So for anyone but the client herself, the day and length come back null and
-- the date is always today. Body taken from the live definition
-- (20260924130000); only the two guarded lines are new.
...
declare
  ...
  v_day      int;
  v_len      int;
  v_self     boolean := p_client = (select auth.uid());
...
begin
  ...
  -- Only she may ask about another day.
  if not v_self then
    p_on := current_date;
  end if;
  ...
  -- The day and the length date a period; they are hers alone.
  if not v_self then
    v_day := null;
    v_len := null;
  end if;
